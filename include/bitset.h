...
//-----------------(variant for test(different realization))-------------
...
//TODO: problem: if numbers is too big we should check it to not overload. if it to small do the same check.(when it out of bounds!!!.
...
public:
    ...
    BITSET operator/(const BITSET& other) const {
        if (this->x > (1 << 21)) {//TODO change it(, becouse it too big(21)
            // Overflow!
